...
class _EcoDexScreenState extends State<EcoDexScreen>
  ...
  // ── Tab state ─────────────────────────────────────────────────────────────
  int _selectedTab = 1; // Identify tab
  ...
  void initState() {
    ...
    _progressController = AnimationController(
      ...
      setState(() => _analysisProgress = _progressController.value);
  ...
  Future<void> _captureAndAnalyze() async {
    ...
    try {
      ...
      _showPixelDialog('ERROR', 'Failed to capture: $e');
    }
  }
  ...
  // ── Fake analysis result (UI demo) ────────────────────────────────────────
  final _fakeResults = [
    {'name': 'Glass Bottle', 'cat': 'Recyclable (Glass)', 'emoji': '🍾', 'tip': 'Rinse before recycling! Glass can be recycled endlessly. Remove metal caps first.'},
    {'name': 'Plastic Bottle', 'cat': 'Recyclable (PET)', 'emoji': '♻️', 'tip': 'Crush and cap it! Check the resin code. PET #1 is widely accepted.'},
    {'name': 'Cardboard Box', 'cat': 'Recyclable (Paper)', 'emoji': '📦', 'tip': 'Flatten boxes to save space. Remove tape and staples before recycling.'},
    {'name': 'Aluminium Can', 'cat': 'Recyclable (Metal)', 'emoji': '🥤', 'tip': 'Aluminium recycling saves 95% energy vs new production. Rinse and crush!'},
    {'name': 'Food Waste', 'cat': 'Compostable', 'emoji': '🌿', 'tip': 'Compost it! Food scraps turn into rich soil. Keep out meat & dairy from home compost.'},
    {'name': 'Styrofoam', 'cat': 'General Waste', 'emoji': '⚠️', 'tip': 'Most styrofoam is not recyclable curbside. Look for specialist drop-off points.'},
  ];

  void _showFakeResult() {
    final result = (_fakeResults..shuffle()).first;
    setState(() {
      _analyzing = false;
      _resultName = result['name'];
      _resultCategory = result['cat'];
  ...
  Widget _buildTopBar() {
    return Container(
      ...
      decoration: const BoxDecoration(
        ...
        border: Border(
          bottom: BorderSide(color: Color(0xFFD8D8B0), width: 2),
        ),
      ),
      child: Row(
        children: [
      ...
      children: [
        ...
        Positioned(
          ...
          child: CustomPaint(
            ...
            painter: _BracketPainter(bracketColor, bracketWidth, corner: 'TL'),
          ),
        ),
        // TR
        Positioned(
          ...
          child: CustomPaint(
            size: const Size(bracketSize, bracketSize),
            painter: _BracketPainter(bracketColor, bracketWidth, corner: 'TR'),
          ),
        ),
        // BL
        Positioned(
          ...
          child: CustomPaint(
            size: const Size(bracketSize, bracketSize),
            painter: _BracketPainter(bracketColor, bracketWidth, corner: 'BL'),
          ),
        ),
        // BR
        Positioned(
          ...
          child: CustomPaint(
            size: const Size(bracketSize, bracketSize),
            painter: _BracketPainter(bracketColor, bracketWidth, corner: 'BR'),
          ),
        ),
      ],
    );
        ...
        builder: (_, __) {
          return Transform.translate(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: _resultName != null
                    ? const Color(0xFF4A7A3A)
                    : const Color(0xFF333333),
                border: Border(
                  top: BorderSide(
                    color: _resultName != null
                        ? const Color(0xFF88CC66)
                        : const Color(0xFF555555),
                    width: 2,
                  ),
                  left: BorderSide(
                ...
                children: [
                  ...
                  if (_resultName != null)
                    Container(
                      ...
                      child: const Center(
                        child: Text('ℹ', style: TextStyle(color: Colors.white, fontSize: 14)),
                      ),
                    ),
                ],
              ),
  ...
  // ── Camera action area (under pokedex) ────────────────────────────────────
  // (Handled via the scan button below)
  ...
  Widget _buildBottomNav() {
    return Container(
      decoration: const BoxDecoration(
        color: Color(0xFFF0F0C8),
        border: Border(
          top: BorderSide(color: Color(0xFFD8D8B0), width: 2),
        ),
      ),
      ...
      child: Row(
        ...
        children: [
          ...
          _NavItem(icon: '📊', label: 'RANK', selected: _selectedTab == 2,
              onTap: () => setState(() => _selectedTab = 2)),
          _NavItem(icon: '👤', label: 'USER', selected: _selectedTab == 3,
...
// Corner bracket painter (for viewfinder)
...
class _IdleScreenPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Pixel plant/leaf art (16x16 grid)
    final pixels = {
      green: [
        [7, 2], [8, 2],
        [6, 3], [7, 3], [8, 3], [9, 3],
